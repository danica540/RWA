...
export class NewsListComponent {
    ...
    returnFullNewsContent(content){
        let divContent="";
        let array=content.split(".");
        array.forEach(sentence=>{
            divContent+="<p>"+sentence+"</p>";
        })
        return divContent;
    }
    ...
    drawNews(news, container) {
        ...
        <div class="card-body">
        <h4 class="bold">${news.headline}</h4>
        <h6 class="bold"> ${news.date}</h6>
    ...
    showSingleNews(news) {
        ...
        <h4 class="bold">${news.headline}</h4>
        <h6 class="bold"> ${news.date}</h6>
        <p class="card-text">${this.returnFullNewsContent(news.content)}</p>
        </div>
        </div>`;
